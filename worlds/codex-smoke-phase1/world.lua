...
local function refreshUi()
    ...
    scoreText.Text = "Blip Mining Draft | " .. table.concat(parts, " | ") .. " | Action mines nearby nodes"
end
...
Client.OnStart = function()
    ...
    scoreText = ui:createText("Blip Mining Draft", Color.White)
...
Client.Tick = function()
    ...
    if marketShape ~= nil then
        ...
        if delta.Length < 8 and inventoryTotal() >= 5 then
            if scoreText ~= nil then
                scoreText.Text = "Market reached. Mining loop complete. Ask chat to expand the world."
            end
        end
    end
end
