...
local resourceNames = { "crystal", "stone", "gold", "sky_crystal", "cloud_stone" }
local resourceNodes = {}
...
local function refreshUi()
    ...
    scoreText.Text = "Blip exploration Draft" .. " | " .. table.concat(parts, " | ") .. " | Action mines nearby nodes"
end
...
Client.OnStart = function()
    ...
    scoreText = ui:createText("Blip exploration Draft", Color.White)
    ...
    Camera.Behavior = {
        ...
        positionTargetOffset = { 0, 10, 0 },
        positionTargetBackoffDistance = 36,
        positionTargetMinBackoffDistance = 18,
        positionTargetMaxBackoffDistance = 72,
        rotationTarget = Player.Head,
        rigidity = 0.35,
    }
end
...
Client.Tick = function()
    ...
    if marketShape ~= nil then
        ...
        if delta.Length < 8 and inventoryTotal() >= 5 then
            if scoreText ~= nil then
                scoreText.Text = "Goal reached. Ask chat to expand this exploration world."
            end
        end
    end
